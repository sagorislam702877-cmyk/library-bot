...
def monthly_category_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("👨‍💼 কর্মী", callback_data="ms|cat|kormi"),
            InlineKeyboardButton("🤝 সাথী", callback_data="ms|cat|sathi"),
        ],
    ...
    for month in MONTH_ORDER:
        cur.append(
            InlineKeyboardButton(
                MONTH_BN[month],
                callback_data=f"ms|month|{category}|{year}|{month}",
            )
        )
        if len(cur) == 2:
            rows.append(cur)
...
def monthly_pdf_keyboard(category: str, year: str, month: str, items: list[dict[str, str]]) -> InlineKeyboardMarkup:
    ...
    for item in items:
        ...
        rows.append([InlineKeyboardButton(f"📘 {short}", callback_data=f"ms|pdf|{item_id}")])
...
def make_inline_keyboard(titles: list[str]) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton(title, callback_data=f"pick|{token}|{i}")] for i, title in enumerate(titles[:10])]
    )
...
def normalize_month(text: Any) -> str:
    ...
    mapping = {
        ...
        "sep": "september", "september": "september", "সেপ্টেম্বর": "september",
    ...
    return mapping.get(raw, normalize(raw))
...
def build_deep_link(monthly_item_id: str) -> str:
    if not monthly_item_id or not BOT_USERNAME:
        return ""
...
def get_sheets():
    ...
    global _cached_monthly_overview_sheet, _cached_monthly_item_sheet
    ...
    try:
        ...
        _cached_log_sheet.append_row(
            ["timestamp", "event", "user_id", "username", "chat_id", "query", "normalized_query", "status", "matched_title", "note"]
        )
    ...
    try:
        ...
        _cached_monthly_overview_sheet.append_row(
            ["category", "year", "month", "monthly_topic", "quran", "hadith", "literature", "class_topic", "discussion_topic", "memorize", "dua"]
        )
    ...
    try:
        ...
        _cached_monthly_item_sheet.append_row(
            ["monthly_item_id", "category", "year", "month", "section", "item_name", "book_id", "book_title_hint"]
        )
        ...
        for year, months in years.items():
            for month, info in months.items():
                rows.append([
                    ...
                    clean_text(info.get("monthly_topic", "")),
                    "; ".join(split_list_text(info.get("quran", []))),
                    "; ".join(split_list_text(info.get("hadith", []))),
                    "; ".join(split_list_text(info.get("literature", []))),
                    "; ".join(split_list_text(info.get("class_topic", []))),
                    "; ".join(split_list_text(info.get("discussion_topic", []))),
                    "; ".join(split_list_text(info.get("memorize", []))),
                    "; ".join(split_list_text(info.get("dua", []))),
                ])
        ...
        for year, months in years.items():
            for month, info in months.items():
                for item in info.get("items", []):
                    rows.append([
                        clean_text(item.get("monthly_item_id", "")),
                        str(category).strip(),
                        str(year).strip(),
                        normalize_month(month),
                        clean_text(item.get("section", "")),
                        clean_text(item.get("item_name", "")),
                        clean_text(item.get("book_id", "")),
                        clean_text(item.get("book_title_hint", "")),
                    ])
...
async def sync_monthly_json_to_sheets() -> tuple[bool, str]:
    ...
    logger.info("SYNC: json fetched")

    overview_values = [["category", "year", "month", "monthly_topic", "quran", "hadith", "literature", "class_topic", "discussion_topic", "memorize", "dua"]]
    overview_values += make_monthly_overview_rows(data_obj)

    item_values = [["monthly_item_id", "category", "year", "month", "section", "item_name", "book_id", "book_title_hint"]]
    item_values += make_monthly_item_rows(data_obj)

    await asyncio.to_thread(overview_sheet.clear)
    await asyncio.to_thread(item_sheet.clear)
    logger.info("SYNC: sheets cleared")
    ...
    logger.info("SYNC: sheets updated")

    MONTHLY_CACHE["ts"] = 0.0
    await get_monthly_cache(force=True)
    logger.info("SYNC: cache refreshed")
...
async def get_monthly_cache(force: bool = False):
    ...
    for row in overview_rows:
        ...
        if cat and year and month:
            overview_lookup[(cat, year, month)] = {
                "category": cat,
                "year": year,
                "month": month,
    ...
    for row in item_rows:
        row = row + [""] * (8 - len(row))
        item_id, cat, year, month, section, item_name, book_id, hint = (
            str(row[0]).strip(),
            normalize(row[1]),
            str(row[2]).strip(),
            normalize_month(row[3]),
            normalize(row[4]),
            row[5].strip(),
            row[6].strip(),
            row[7].strip(),
...
async def syncplan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.exception("SYNCPLAN FAILED: %s", e)
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"ইউজার: {len(USER_IDS)}\n"
        f"বই: {len(BOOK_CACHE['rows'])}\n"
        f"মাসিক সেকশন: {len(overview)}\n"
...
async def process_book_search(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_text_raw: str, user_id: int, username: str = ""):
    ...
    if matched:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="📚 অনেকগুলো বই পাওয়া গেছে। নিচ থেকে বেছে নিন:",
    ...
    if close_keys:
        ...
        if suggestions:
            await context.bot.send_message(
                chat_id=chat_id,
                text="🤖 সম্ভাব্য বইগুলো:",
                reply_markup=make_inline_keyboard(suggestions[:10]),
            )
            return
...
async def send_monthly_message(message_target, category: str, year: str, month: str, target_item_id: str = ""):
    ...
    html = "\n".join([
        ...
        "━━━━━━━━━━━━━━━━━━",
        "",
        "📥 যদি PDF চাও, তাহলে বইগুলোর নামের উপর ক্লিক করো বা নিচের বাটনে ক্লিক করো।",
    ])
