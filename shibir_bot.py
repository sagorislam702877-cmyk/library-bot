...
import asyncio import logging import os import re import time from dataclasses import dataclass, field from difflib import get_close_matches from threading import Thread from typing import Any, Optional

import gspread from flask import Flask from google.oauth2.service_account import Credentials from telegram import InlineKeyboardButton, InlineKeyboardMarkup, Update from telegram.error import BadRequest from telegram.ext import Application, CallbackQueryHandler, CommandHandler, ContextTypes, MessageHandler, filters

================= CONFIG =================

BOT_TOKEN = os.environ.get("BOT_TOKEN", "").strip() ADMIN_ID = int(os.environ.get("ADMIN_ID", "0")) SHEET_NAME = os.environ.get("SHEET_NAME", "MyBotDB").strip() LOG_SHEET_NAME = os.environ.get("LOG_SHEET_NAME", "Logs").strip()

if not BOT_TOKEN: raise RuntimeError("BOT_TOKEN is missing") if ADMIN_ID == 0: raise RuntimeError("ADMIN_ID is missing or invalid")

================= PERFORMANCE TUNING =================

Free resource friendly defaults.

Lower worker count = less CPU, more fairness.

DELIVERY_WORKERS = max(1, int(os.environ.get("DELIVERY_WORKERS", "4"))) SEND_CONCURRENCY = max(1, int(os.environ.get("SEND_CONCURRENCY", "6"))) BOOK_CACHE_TTL = int(os.environ.get("BOOK_CACHE_TTL", "300")) CALLBACK_CACHE_TTL = int(os.environ.get("CALLBACK_CACHE_TTL", "600")) LOG_FLUSH_INTERVAL = int(os.environ.get("LOG_FLUSH_INTERVAL", "8")) USER_FLUSH_INTERVAL = int(os.environ.get("USER_FLUSH_INTERVAL", "8")) BOOK_REFRESH_INTERVAL = int(os.environ.get("BOOK_REFRESH_INTERVAL", "300")) MAX_INLINE_SUGGESTIONS = 10 MAX_DIRECT_SEND = 5 MAX_SEARCH_RESULTS_FOR_KEYBOARD = 10 MAX_BATCH_LOGS = 100 MAX_BATCH_USERS = 50

================= APP STATE =================

LOG_QUEUE: list[list[str]] = [] NEW_USER_QUEUE: set[int] = set()

_cached_book_sheet = None _cached_user_sheet = None _cached_log_sheet = None _sheets_lock = asyncio.Lock() _book_cache_lock = asyncio.Lock() _callback_cache_lock = asyncio.Lock()

_callback_cache: dict[str, dict[str, Any]] = {} _admin_reply_map: dict[int, int] = {} _chat_send_locks: dict[int, asyncio.Lock] = {}

DELIVERY_QUEUE: Optional[asyncio.Queue] = None DELIVERY_STARTED = False SEND_GATE = asyncio.Semaphore(SEND_CONCURRENCY)

BOOK_CACHE = { "ts": 0.0, "rows": [], "indexed": [], "titles": [], "lookup": {}, }
...
@web_app.route("/") def home(): return "Library Bot is Running!"

def run_web(): port = int(os.environ.get("PORT", 8080)) import logging as flask_logging
...
def _build_credentials() -> Credentials: scope = [ "https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive", ] return Credentials.from_service_account_file("creds.json", scopes=scope)

async def get_sheets(): global _cached_book_sheet, _cached_user_sheet, _cached_log_sheet
...
async with _sheets_lock:
    ...
    try:
        ...
        if _cached_log_sheet is None:
            ...
            # Header only once when log sheet is new/empty-ish.
            try:
...
async def flush_logs_once(): _, _, log_sheet = await get_sheets() if not log_sheet or not LOG_QUEUE: return
...
async def flush_users_once(): _, user_sheet, _ = await get_sheets() if not user_sheet or not NEW_USER_QUEUE: return
...
async def background_sync_task(): while True: await asyncio.sleep(min(LOG_FLUSH_INTERVAL, USER_FLUSH_INTERVAL)) await flush_logs_once() await flush_users_once()

async def refresh_book_cache_periodically(): while True: await asyncio.sleep(BOOK_REFRESH_INTERVAL) await get_book_cache(force=True)

async def cleanup_callback_cache_periodically(): while True: await asyncio.sleep(60) now = time.time() async with _callback_cache_lock: expired = [k for k, v in _callback_cache.items() if now - float(v.get("ts", 0.0)) > CALLBACK_CACHE_TTL] for key in expired: _callback_cache.pop(key, None)

================= CACHE HELPERS =================

def invalidate_book_cache(): BOOK_CACHE["ts"] = 0.0

async def get_book_cache(force: bool = False): now = time.time() if (not force) and BOOK_CACHE["rows"] and (now - BOOK_CACHE["ts"] < BOOK_CACHE_TTL): return BOOK_CACHE["rows"], BOOK_CACHE["indexed"], BOOK_CACHE["titles"], BOOK_CACHE["lookup"]
...
async def load_users_initial(): _, user_sheet, _ = await get_sheets() if not user_sheet: return try: values = await asyncio.to_thread(user_sheet.col_values, 1) ids = {int(str(x).strip()) for x in values[1:] if str(x).strip().isdigit()} USER_CACHE["ids"] = ids USER_CACHE["ts"] = time.time() except Exception as e: logging.error(f"User load error: {e}")

================= TEXT UTIL =================

def normalize(text): if not text: return "" text = str(text).lower().strip() return re.sub(r"[\s\W_]+", "", text, flags=re.UNICODE)

def tokenize(text): return re.findall(r"[\w\u0980-\u09FF]+", str(text).lower(), flags=re.UNICODE)

def clean_title_like_text(text): text = str(text).strip() text = re.sub(r"^[-*\d.)\s]+", "", text) return re.sub(r"\s+", " ", text).strip()

def is_url_like(text): return bool(re.search(r"(https?://|www.|t.me/|telegram.me/)", str(text), flags=re.I))

def remove_urls(text): return re.sub(r"(https?://\S+|www.\S+|t.me/\S+|telegram.me/\S+)", "", str(text), flags=re.I)

================= AUTO ALIAS GENERATION =================

BENGALI_LATIN_MAP = { "অ": "a", "আ": "a", "ই": "i", "ঈ": "i", "উ": "u", "ঊ": "u", "ঋ": "ri", "এ": "e", "ঐ": "oi", "ও": "o", "ঔ": "ou", "ক": "k", "খ": "kh", "গ": "g", "ঘ": "gh", "ঙ": "ng", "চ": "ch", "ছ": "chh", "জ": "j", "ঝ": "jh", "ঞ": "ny", "ট": "t", "ঠ": "th", "ড": "d", "ঢ": "dh", "ণ": "n", "ত": "t", "থ": "th", "দ": "d", "ধ": "dh", "ন": "n", "প": "p", "ফ": "f", "ব": "b", "ভ": "bh", "ম": "m", "য": "y", "র": "r", "ল": "l", "শ": "sh", "ষ": "sh", "স": "s", "হ": "h", "ড়": "r", "ঢ়": "rh", "য়": "y", "ং": "ng", "ঃ": "h", "ঁ": "n", "ৎ": "t", "া": "a", "ি": "i", "ী": "i", "ু": "u", "ূ": "u", "ে": "e", "ৈ": "oi", "ো": "o", "ৌ": "ou", "্": "", }

def transliterate_bengali_to_latin(text): if not text: return "" out = [BENGALI_LATIN_MAP.get(ch, ch) for ch in str(text)] result = "".join(out) result = re.sub(r"[^a-zA-Z0-9\s]+", " ", result) return re.sub(r"\s+", " ", result).strip().lower()

def generate_auto_aliases(title): aliases = set() if not title: return aliases
...
def parse_manual_aliases(value): aliases = set() if not value: return aliases for part in re.split(r"[,;\n|/]+", str(value)): part = part.strip() if part: aliases.add(normalize(part)) aliases.update(generate_auto_aliases(part)) return aliases

def build_search_index(rows): lookup = {} titles = [] indexed = []
...
async def append_log(event, user_id, username, chat_id, query, normalized_query, status, matched_title="", note=""): row = [ time.strftime("%Y-%m-%d %H:%M:%S"), event, str(user_id or ""), str(username or ""), str(chat_id or ""), str(query or ""), str(normalized_query or ""), str(status or ""), str(matched_title or ""), str(note or ""), ] LOG_QUEUE.append(row)

async def ensure_user_saved(user_id): if user_id not in USER_CACHE["ids"]: USER_CACHE["ids"].add(user_id) NEW_USER_QUEUE.add(user_id)

================= KEYBOARD =================

def make_inline_keyboard(titles): token = str(time.time_ns())[-10:] titles = titles[:MAX_INLINE_SUGGESTIONS] _callback_cache[token] = {"titles": titles, "ts": time.time()} keyboard = [[InlineKeyboardButton(title, callback_data=f"pick|{token}|{i}")] for i, title in enumerate(titles)] return InlineKeyboardMarkup(keyboard)

================= DELIVERY SYSTEM =================

@dataclass class DeliveryJob: chat_id: int user_id: int username: str rows: list[list[str]] query_text: str normalized_query: str note: str = "" progress: int = 0 total: int = 0 created_at: float = field(default_factory=time.time)

def __post_init__(self):
    self.total = len(self.rows)

async def get_chat_lock(chat_id: int) -> asyncio.Lock: if chat_id not in _chat_send_locks: _chat_send_locks[chat_id] = asyncio.Lock() return _chat_send_locks[chat_id]

async def send_one_document(bot, chat_id: int, file_id: str, caption: str): async with SEND_GATE: await bot.send_document(chat_id=chat_id, document=file_id, caption=caption)

async def delivery_worker(application: Application, worker_no: int): assert DELIVERY_QUEUE is not None
...
while True:
    ...
    try:
        ...
        async with chat_lock:
            # One round per pass keeps the whole bot fair.
            # A large job yields back to the queue so other users do not wait.
            ...
            while job.progress < job.total and sent_this_round < round_limit:
                ...
                # tiny pause helps rate-limit safety while keeping perceived speed high
...
async def start_delivery_system(application: Application): global DELIVERY_QUEUE, DELIVERY_STARTED
...
async def queue_book_delivery(context, chat_id: int, rows, user_id: int, username: str, query_text: str, normalized_query: str, note: str = ""): total = len(rows) ack = "📚 বই পাওয়া গেছে, পাঠানো শুরু করছি..." if total == 1 else f"📚 {total}টি বই পাওয়া গেছে, পাঠানো শুরু করছি..." await context.bot.send_message(chat_id=chat_id, text=ack)
...
if DELIVERY_QUEUE is None:
    # Safety fallback. Should not happen after startup.
...
async def send_books_by_rows(bot, chat_id: int, rows, batch_size: int = 3, batch_pause: float = 0.25): sent, seen = 0, set() chat_lock = await get_chat_lock(chat_id)
